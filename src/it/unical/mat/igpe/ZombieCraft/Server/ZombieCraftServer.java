...
public class ZombieCraftServer extends SimpleApplication implements
		ConnectionListener, AnimEventListener {

	public static JFileChooser modifyChooser;
	private static int port;
	private static File mapDirectory = new File(System.getProperty("user.home")
			+ "/Documents/ZombieMap");

	private static final Vector3f lightDirection = new Vector3f(-0.8F, -1.0F,
			-0.8F).normalizeLocal();
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (option == JFileChooser.APPROVE_OPTION) {
				...
				Utility.filePath = modifyChooser.getSelectedFile()
						.getAbsolutePath();

				ZombieCraftServer zombieServer = new ZombieCraftServer();
				...
				int response = JOptionPane.showOptionDialog(null,
						"Server or Spectator mode?", "Server Camera Options",
	...
	public void simpleInitApp() {
		...
		try {
			...
			int response = JOptionPane.showOptionDialog(null,
					...
					"Server Options", JOptionPane.DEFAULT_OPTION,
	...
	public void initGame() {
		...
		initSpawners();
		load();
		assetManager.registerLocator("ProjectAssets/",
	...
	private boolean checkCollisionWithTerrain(Spatial a, Spatial b) {
		...
		if (v1.x >= (v2.x - 1) && v1.x <= (v2.x + 1) && v1.z >= (v2.z - 1)
				&& v1.z >= (v2.z + 1)) {
	...
	public void dropPowerUps(float tpf) {
		...
		if (clientSpatials.size() > 0) {
			...
			if (timerAmmo > 8) {
				...
				Vector3f location = new Vector3f(random.nextInt(300) + 1, 200F,
						random.nextInt(300) + 1);
				Vector3f direction = new Vector3f(0.0F, -1.0F, 0.0F);

				Vector3Int blockLocation = getCurrentPointedBlockLocation1(
						true, location, direction);
	...
	private Vector3Int getCurrentPointedBlockLocation1(
			boolean getNeighborLocation, Vector3f location, Vector3f direction) {
		CollisionResults results = getRayCastingResults1(terrainNode, location,
				direction);
		if (results.size() > 0) {
			Vector3f collisionContactPoint = results.getClosestCollision()
					.getContactPoint();
			return BlockNavigator.getPointedBlockLocation(blockTerrain,
					collisionContactPoint, getNeighborLocation);
		}
		return null;
	}

	private CollisionResults getRayCastingResults1(Node node,
			Vector3f location, Vector3f direction) {
	...
	private void initBlockTerrain() {
		...
		blockTerrain = new BlockTerrainControl(getSettings(this),
				new Vector3Int(5, 1, 5));
		blockTerrain.setBlockArea(new Vector3Int(1, 1, 1), new Vector3Int(100,
				1, 100), Utility.DIRT_CUBE);
		...
		blockTerrain.addChunkListener(new BlockChunkListener() {
			...
			public void onSpatialUpdated(BlockChunkControl blockChunk) {
				...
				RigidBodyControl rigidBodyControl = optimizedGeometry
				...
				rigidBodyControl.setCollisionShape(new MeshCollisionShape(
	...
	private void initCubes() {
		...
		cubes = new Cube[] { dirtCube, woodCube, brickCube, stoneCube, iceCube,
				glassCube, enemySpawnerCube };
	}
	...
	private void initSpawners() {
		...
		EnemySpawnerCube enemySpawnerCube = new EnemySpawnerCube(
		...
		EnemySpawnerCube enemySpawnerCube2 = new EnemySpawnerCube(
				new Vector3Int(230, 25, 0));

		enemySpawners.add(enemySpawnerCube2.getEnemySpawner());
		// blockTerrain.setBlock(enemySpawnerCube2.getPosition(),
		// enemySpawnerCube2);

		EnemySpawnerCube enemySpawnerCube3 = new EnemySpawnerCube(
				new Vector3Int(230, 25, 230));

		enemySpawners.add(enemySpawnerCube3.getEnemySpawner());
		// blockTerrain.setBlock(enemySpawnerCube3.getPosition(),
		// enemySpawnerCube3);

		EnemySpawnerCube enemySpawnerCube4 = new EnemySpawnerCube(
				new Vector3Int(0, 25, 235));
		...
		for (int i = 0; i < enemies.size(); i++) {
			if (clientSpatials.containsKey(id)) {
				if (enemies.get(i).getHealthPoints() > 0) {
					if (checkCollision(clientSpatials.get(id),
							enemySpatials.get(i))
							&& enemies.get(i).canAttack()) {
						...
						enemyMessage.setPosition(enemySpatial
								.getLocalTranslation());
						enemyMessage.setRotation(enemySpatial
								.getLocalRotation());
						enemyMessage.setAnimation("Attack");
						enemyMessage.setEnemySound("Attack");
						enemyMessage.setLinearVelocity(enemySpatial.getControl(
								RigidBodyControl.class).getLinearVelocity());
	...
	private void hitPlayer(int damage, int id) {
		...
		if (clientPlayers.get(id).getHealthPoints() <= 0
				|| clientPlayers.get(id).getPosition().y <= -15) {
	...
	private void load() {
		...
		if (file.exists()) {
			...
			for (int i = 0; i < _splitNumbers; i++) {
				...
				for (int j = 0; j < 1024 && position < (i * 1024)
						&& position < bFile.length; j++, position++)
			...
			server.addMessageListener(serverListener, MapMessage.class,
					PlayerMessage.class, HelloMessage.class,
					PlayerListMessage.class, EnemyMessage.class,
					PowerupMessage.class);
		}
	}
	...
	private void makeEnemy(Vector3Int position) {
		...
		if (clientSpatials.size() > 0) {
			...
			enemySpatial = assetManager
					.loadModel("Zombie/ZombieCraftPlayerANIMATED.scene");

			MaterialList matList = (MaterialList) assetManager
					.loadAsset("Zombie/Player.material");
			...
			RigidBodyControl enemyCharacterControl = new RigidBodyControl(
					enemyCollisionShape);
			...
			int choice = ThreadLocalRandom.current().nextInt(0,
					clientSpatials.size());
			if (clientSpatials.get(choice) != null
					&& clientPlayers.get(choice).getHealthPoints() > 0)
	...
	public void simpleUpdate(float tpf) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Server crashed because of network problems");
			this.stop();
		}
	}
	...
	private void updateClients(float tpf) {
		if (serverListener.getPlayerMessageQueue() != null) {
			for (int key : serverListener.getPlayerMessageQueue().keySet()) {
				PlayerMessage message = serverListener.getPlayerMessageQueue()
						.get(key);
				if (message.getPosition() != null
						&& message.getRotation() != null) {
					...
					if (sp != null) {
						if (p.getHealthPoints() <= 0
							...
							if (playerAnimChannel.getTime() == playerAnimChannel
									.getAnimMaxTime())
							...
							for (Vector3f _key : powerups.keySet()) {
								if (Math.abs(_key.x - playerPosition.x) < 5f
									...
									if (powerups.get(_key) instanceof AmmoCube) {
										AmmoCube ammoCube = (AmmoCube) powerups
												.get(_key);
										Vector3Int blockLocation = ammoCube
												.getBlockLocation();
										...
										powerupMessage.setType(ammoCube
												.getClass().getName());
										powerupMessage.setX(blockLocation
												.getX());
										powerupMessage.setY(blockLocation
												.getY());
										powerupMessage.setZ(blockLocation
												.getZ());
										powerupMessage.setAmount(25);
										server.broadcast(powerupMessage);

										blockTerrain
												.removeBlock(((AmmoCube) powerups
														.get(_key))
														.getBlockLocation());
										...
										HealthCube healthCube = (HealthCube) powerups
												.get(_key);
										Vector3Int blockLocation = healthCube
												.getBlockLocation();
										...
										PowerupMessage powerupMessage = new PowerupMessage();
										powerupMessage.setDelete(true);
										powerupMessage.setType(healthCube
												.getClass().getName());
										powerupMessage.setX(blockLocation
												.getX());
										powerupMessage.setY(blockLocation
										...
										blockTerrain
												.removeBlock(((HealthCube) powerups
														.get(_key))
														.getBlockLocation());
									}
							...
							if (Vector3f.isValidVector(message.getShootPoint())) {
								...
								for (int i = 0; i < enemySpatials.size(); i++) {
									mark.setLocalTranslation(message
											.getShootPoint());

									if (checkCollision(mark,
											enemySpatials.get(i))) {
										enemies.get(i).decreaseHealthPoints(
												message.getDamage());
		...
		if (serverListener.getPlayerListMessages() != null) {
			...
			for (int key : serverListener.getPlayerListMessages().keySet()) {
				...
				if (!clientPlayers.containsKey(key)) {
					Player p = new Player();
					p.setName(serverListener.getPlayerListMessages().get(key)
							.getName());

					// draw player
					MaterialList matList = (MaterialList) assetManager
							.loadAsset("PlayerNew/Player.material");
					OgreMeshKey ogreKey = new OgreMeshKey(
							"PlayerNew/Cube.mesh.xml", matList);
					Spatial sp = assetManager
							.loadModel("PlayerNew/ZombieCraftPlayerWithWeapon.scene");
					...
					sp.addLight(new AmbientLight());
					CharacterControl playerCharacterControl = new CharacterControl(
							new CapsuleCollisionShape(getSettings(this)
									.getBlockSize() / 2.0F, getSettings(this)
									.getBlockSize() * 2.0F), 0.05F);
					...
					clientPlayers.put(serverListener.getPlayerListMessages()
							.get(key).getId(), p);
					clientSpatials.put(serverListener.getPlayerListMessages()
							.get(key).getId(), sp);
	...
	private void updateSpawner(float lastTimePerFrame) {
		...
		for (int i = 0; i < enemySpawners.size(); i++) {
			...
			if (spawnTimeCounter >= tmp.getSpawnTime()
					&& tmp.getCurrentSlots() < tmp.getMaxSlots())
			...
			for (int i = 0; i < enemies.size(); i++) {
				...
				if (e.getHealthPoints() <= 0 || e.getY() < -15) {
					...
					for (int l = 0; l < terrainNode.getChildren().size(); l++)
						if (checkCollisionWithTerrain(eS,
								terrainNode.getChild(l)))
							eS.getControl(RigidBodyControl.class)
									.setLinearVelocity(new Vector3f(0, 6, 0));
					...
					if (clientSpatials.size() > 0 && e.getTarget() == null) {
						int choice = ThreadLocalRandom.current().nextInt(0,
								clientPlayers.size());
						if (clientPlayers.get(choice) != null
								&& clientPlayers.get(choice).getHealthPoints() > 0) {
					...
					if (e.getTarget() != null) {
						...
						RigidBodyControl rb = eS
								.getControl(RigidBodyControl.class);

						Vector3f v = new Vector3f(0, rb.getLinearVelocity().y,
								0);
						...
						if (e.getTarget() != null
								&& e.getTarget().getPosition() != Vector3f.NAN) {

							Vector3f monsterLocation = eS.getLocalTranslation();

							Vector3f playerDirection = playerPosition
									.subtract(monsterLocation);

							playerDirection.y = 0;

							Quaternion targetRotation = new Quaternion();

							targetRotation.lookAt(playerDirection,
									Vector3f.UNIT_Y);
				...
				enemyMessage.setLinearVelocity(eS.getControl(
						RigidBodyControl.class).getLinearVelocity());
	...
	public static void initializeWater(SimpleApplication simpleApplication) {
		WaterFilter waterFilter = new WaterFilter(
				simpleApplication.getRootNode(), lightDirection);
	...
	public static void initializeEnvironment(SimpleApplication simpleApplication) {
		...
		simpleApplication.getRootNode().attachChild(
				SkyFactory.createSky(simpleApplication.getAssetManager(),
						"Textures/cubes/sky.jpg", true));
		...
		simpleApplication.getViewPort().addProcessor(
				directionalLightShadowRenderer);
	}

	private static FilterPostProcessor getFilterPostProcessor(
			SimpleApplication simpleApplication) {
		List<SceneProcessor> sceneProcessors = simpleApplication.getViewPort()
				.getProcessors();
